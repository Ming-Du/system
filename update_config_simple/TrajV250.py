...
instanceCacheUtils = CacheUtils("/home/mogo/data/trajectory_agent_cache_record_V250.json")
...
def CheckFileExistsByXAndY(floatStartX, floatStartY, floatEndX, floatEndY):
    ret = EnumTrajResult.ENUMTRAJRESULT_V250_SEARCH_LAT_LON_FAILED
    try:
        strFileName1 = "/home/mogo/data/vehicle_monitor/MapEngine_data/track_record_data/{4}/traj_{0}_{1}_{2}_{3}.csv".format(
            subFloat(floatStartX), subFloat(floatStartY), subFloat(floatEndX), subFloat(floatEndY), getCarDirName())
        strFileName2 = "/home/mogo/data/vehicle_monitor/MapEngine_data/track_record_data/{4}/stop_{0}_{1}_{2}_{3}.txt".format(
            subFloat(floatStartX), subFloat(floatStartY), subFloat(floatEndX), subFloat(floatEndY), getCarDirName())
        rospy.logwarn("strFileName1:{0}".format(strFileName1))
        rospy.logwarn("strFileName2:{0}".format(strFileName2))
        if os.path.exists(strFileName1) and os.path.exists(strFileName2):
            ret = EnumTrajResult.ENUMTRAJRESULT_V250_SEARCH_LAT_LON_SUCCESS
            rospy.loginfo("v250 xy file exists")
        else:
            rospy.loginfo("v250 xy file not exists")
            ret = EnumTrajResult.ENUMTRAJRESULT_V250_SEARCH_LAT_LON_FAILED
    except Exception as e:
        rospy.logwarn('repr(e):{0}'.format(repr(e)))
        rospy.logwarn('e.message:{0}'.format(e.message))
...
class TrajV250(InterfaceDataSource):
    strUrlList = None
    strUrlSync = None
    mScheduler = None
    ...
    def __init__(self):
        rospy.loginfo("enter TrajV250::__init__")
        try:
            self.mEnumDataSourceType = EnumDataSourceType.DATA_SOURCE_UPDATE_CONFIG
            self.mStrConfigFileName = "/home/mogo/data/TrajectoryConfigCache_V250.json"
    ...
    def setScheduler(self, instanceScheduler):
        rospy.loginfo("enter TrajV250::setScheduler")
        try:
    ...
    def setCacheUtils(self, instanceCacheUtil):
        rospy.loginfo("enter TrajV250::setCacheUtils")
        try:
            rospy.loginfo("enter TrajV250::setCacheUtils")
    ...
    def getModuleName(self):
        return "df_trajectory_250"

    def getVersion(self):
        pass

    def configure(self):
        rospy.loginfo("enter TrajV250::configure")
        intError = 0
        try:
    ...
    def init_module(self):
        rospy.loginfo("enter TrajV250::init_module")
    ...
    def topicMsgCallbackStartup(self, msg, lLineId, strTrajUrl, strTrajMd5, strStopUrl, strStopMd5, floatStartX,
        ...
        try:
            rospy.loginfo("enter TrajV250::topicMsgCallbackStartup")
            rospy.loginfo(
                "TrajV250::topicMsgCallbackStartup lLineId:{0}, strTrajUrl:{1}, strTrajMd5:{2}, strStopUrl:{3}, strStopMd5:{4}, floatStartX:{5}, floatStartY:{6}, floatEndX:{7}, floatEndY:{8})".format(
    ...
    def call_para(self, lLineId, strTrajUrl, strTrajMd5, strStopUrl, strStopMd5, timestamp, floatStartX,
                  floatStartY, floatEndX, floatEndY):
        intProcessRet = EnumTrajResult.ENUMTRAJRESULT_V250_SEARCH_LAT_LON_FAILED
        try:
            rospy.loginfo("enter TrajV250::call_para")
            rospy.loginfo(
            ...
            while True:
                if int(lLineId) > 0:
                    strStandardLocationFileStop = "/home/mogo/data/vehicle_monitor/MapEngine_data/track_record_data/{1}/stop_{0}.txt".format(
                        lLineId,getCarDirName())
                    strStandardLocationFileTraj = "/home/mogo/data/vehicle_monitor/MapEngine_data/track_record_data/{1}/traj_{0}.csv".format(
                        lLineId,getCarDirName())
                ...
                if int(timestamp) > 0 or len(strTrajUrl.strip()) == 0 or len(strTrajMd5.strip()) == 0 or len(
                        strStopUrl.strip()) == 0 or len(strStopMd5.strip()) == 0 and float(floatStartX) > float(
                    0.0) and float(floatStartY) > float(0.0) and float(floatEndX) > float(
                    ...
                    if os.path.exists(strStandardLocationFileTraj) and os.path.exists(strStandardLocationFileStop):
                        intProcessRet = EnumTrajResult.ENUMTRAJRESULT_V250_TRAJ_WARNING
    ...
    def transaction(self, lLineId, strTrajUrl, strTrajMd5, strStopUrl, strStopMd5, timestamp, floatStartX, floatStartY,
                    floatEndX, floatEndY):
        try:
            rospy.loginfo("enter TrajV250::transaction")
            rospy.loginfo(
                "call_process: recv  lLineId:{0}, strTrajUrl:{1}, strTrajMd5:{2}, strStopUrl:{3},strStopMd5:{4},timestamp:{5}".format(
    ...
    def getNeedUpdateFile(self, refJob):
        rospy.loginfo("enter TrajV250::getNeedUpdateFile")
        intError = 0
        try:
            if len(refJob) == 0:
                intError = -1
    ...
    def pushJobScheduler(self, refDataSource, refJob):
        try:
            rospy.loginfo("enter TrajV250::pushJobScheduler")
    ...
    def schedulerFinishAction(self, refJob):
        try:
            rospy.loginfo("enter TrajV250::schedulerFinishAction")
            self.checkAtomicFeature(refJob)
            self.install_stage_path(refJob)
            intResult = self.notify_pad(refJob)
            rospy.loginfo("schedulerFinishAction intResult: {0}".format(intResult))
            if intResult == EnumTrajResult.ENUMTRAJRESULT_V250_DOWNLOAD_SUCCESS:
            ...
            self.mCallerHandler.subRequestFinish("V250", intResult)
        except Exception as e:
            rospy.logwarn('repr(e):{0}'.format(repr(e)))
            rospy.logwarn('e.message:{0}'.format(e.message))
            rospy.logwarn('traceback.format_exc():%s' % (traceback.format_exc()))
    ...
    def checkAtomicFeature(self, refJob):
        rospy.loginfo("enter TrajV250::checkAtomicFeature")
        ret = 0
        try:
            for idx in range(len(refJob.listJobCollectUpdate)):
                ...
                if not os.path.exists(refJob.listJobCollectUpdate[idx].strFullFileTempName):
                    rospy.logwarn("TrajV250::checkAtomicFeature tempFileNotExists:{0}".format(refJob.listJobCollectUpdate[idx].strFullFileTempName))
                    continue
                ...
                ret, strOutputFilePath = instanceCommonUtilsCompare.fromBase64Restore(strInputBase64FileName, strOriginFileName)
                if ret == 0:
                    strRealMd5 = instanceCommonUtilsCompare.checkFileMd5(strOutputFilePath)
                    rospy.logwarn("TrajV250::checkAtomicFeature strStandardMd5:{0},realfile:{1}_strRealMd5:{2}".format(strStandardMd5,strOriginFileName,strRealMd5))
                    if strRealMd5 == strStandardMd5:
                        refJob.listJobCollectUpdate[idx].intStatus = 0
                        rospy.logwarn("TrajV250::checkAtomicFeature strFileName:{0} matched md5".format(strOutputFilePath))
                    if strRealMd5 != strStandardMd5:
                        refJob.listJobCollectUpdate[idx].intStatus = -1
                        rospy.logwarn("TrajV250::checkAtomicFeature strFileName:{0} not  matched md5".format(strOutputFilePath))
                else:
                    rospy.logwarn("TrajV250::checkAtomicFeature instanceCommonUtilsCompare.fromBase64Restore:{0}".format(ret))
                    refJob.listJobCollectUpdate[idx].intStatus = -1
    ...
    def install_stage_path(self, refJob):
        rospy.loginfo("enter TrajV250::install_stage_path")

    def install_dst_path(self, refJob):
        try:
            rospy.loginfo("enter TrajV250::install_dst_path")
    ...
    def write_cache_file(self, refJob):
        rospy.loginfo("enter TrajV250::write_cache_file")
        try:
            for idx in range(len(refJob.listJobCollect)):
    ...
    def check_local_files(self, refJob):
        rospy.loginfo("enter TrajV250::check_local_files")
        intResultProcess = EnumTrajResult.ENUMTRAJRESULT_V250_DOWNLOAD_FAILED
        try:
            while True:
                if refJob is None:
                ...
                if os.path.exists(refJob.listJobCollect[0].strFullFileName) and os.path.exists(
                    ...
                    intResultProcess = EnumTrajResult.ENUMTRAJRESULT_V250_TRAJ_WARNING
                    break
                break
        except Exception as e:
            rospy.logwarn('repr(e):{0}'.format(repr(e)))
    ...
    def notify_pad(self, refJob):
        rospy.loginfo("enter TrajV250::notify_pad")
        try:
            intResultProcess = EnumTrajResult.ENUMTRAJRESULT_V250_DOWNLOAD_FAILED
            intSuccessNum = 0
            for idx in range(len(refJob.listJobCollectUpdate)):
            ...
            while True:
                ...
                if intSuccessNum == 2 and len(refJob.listJobCollectUpdate) == 2:
                    intResultProcess = EnumTrajResult.ENUMTRAJRESULT_V250_DOWNLOAD_SUCCESS
                    break
    ...
    def notify_cloud(self, refJob):
        rospy.loginfo("enter TrajV250::notify_cloud")

    def write_event(self, refJob):
        rospy.loginfo("enter TrajV250::write_event")
    ...
    def setCallerHandler(self, handler):
        rospy.loginfo("enter TrajV250::setCallerHandler")
