...
class CacheUtils:
    ...
    def WriteFileCacheInfo(self, lLineId, strTrajUrl, strTrajMd5, strStopUrl, strStopMd5, timestampTraj, timestampStop,
        ...
        rospy.logdebug("process WriteFileCacheInfo: lLineId:{0}, strTrajUrl:{1}, strTrajMd5:{2},strStopUrl:{3},strStopMd5:{4},timestampTraj:{5},timestampStop:{6},intModifyTime_Traj:{7},intModifyTime_Stop:{8}".format(
        ...
        try:
            ...
            rospy.logdebug("strTrajName:{0}".format(strTrajName))
            ...
            rospy.logdebug("strStopName:{0}".format(strStopName))
            ...
            rospy.logdebug("=======================current dictTrajectoryAgentRecord:{0}".format(
        ...
        try:
            ...
            if os.path.exists(strFileName):
                ...
                if len(self.dictTrajectoryAgentRecord) > 0:
                    rospy.logdebug("restore success")
                    rospy.logdebug("content:{0}".format(json.dumps(self.dictTrajectoryAgentRecord)))
...
def downFileFromUrlWget(strUrl, strTempFileName):
    ...
    strWgetCmd = " /usr/bin/wget     --connect-timeout=5 --dns-timeout=5  -c    '{0}'  -O   '{1}' ".format(
        ...
        strTempFileName)
    try:
        rospy.logdebug("execute sub cmd : {0}".format(strWgetCmd))
    ...
    try:
        ...
        if len(content) > 0:
            ...
            if dictContent.has_key('errno'):
                ...
                rospy.logdebug ("====================================================================================strUrl:{0},intErrno:{1}".format(
                    strUrl, intErrno))
            if (intErrno == 0) and (dictContent.has_key('result')):
                rospy.logdebug ("-------------------enter  (intErrno == 0) and (dictContent.has_key('result')) and  strTempFileName:{0} ".format(
                    strTempFileName))
    ...
    try:
        while True:
            ...
            if try_times > global_MaxTryTimes:
                rospy.logwarn("Exit  Download  , Download file try times:{0} more than global_MaxTryTimes:{1}".format(try_times,
                                                                                                              global_MaxTryTimes))
                retNum = -1
                break
...
def processFile(lLineId, strTrajUrl, strTrajMd5, strStopUrl, strStopMd5, timestamp):
    rospy.loginfo("--------------------------processFile: lLineId:{0}, strTrajUrl:{1}, strTrajMd5:{2}, strStopUrl:{3}, strStopMd5:{4}, timestamp:{5}".format(
    ...
    if len(strTrajUrl.strip()) == 0:
        rospy.logdebug("processFile recv para error strTrajUrl")
        intProcessRet = 1

    if len(strTrajMd5.strip()) == 0:
        rospy.logdebug("processFile recv para error strTrajMd5")
        intProcessRet = 1

    if len(strStopUrl.strip()) == 0:
        rospy.logdebug("processFile recv para error strStopUrl")
        intProcessRet = 1

    if len(strStopMd5.strip()) == 0:
        rospy.logdebug("processFile recv para error strStopMd5")
        intProcessRet = 1
        ...
        try:
            ...
            while True:
                ...
                if os.path.exists(strStandardLocationFileTraj) == False:
                    rospy.logdebug("########## enter switch not g_CacheUtil.CheckTrajFileCacheExists(lLineId, timestamp, strTrajMd5)")
                ...
                if os.path.exists(strStandardLocationFileStop) == False :
                    rospy.logdebug("########## enter switch not g_CacheUtil.CheckStopFileCacheExists(lLineId, timestamp, strStopMd5)")
                    ## down to temp  location
                    if globalDictTaskRunningStatus != lLineId:
                        break
                    intCheckStatus = 0
                    ...
                    break
                # os.__exit(-1)
                rospy.logdebug("--------------------------before switch (g_CacheUtil.CheckTrajFileCacheExists(lLineId, timestamp, strTrajMd5) == True) and (os.path.exists(strStandardLocatio)")
                if (g_CacheUtil.CheckTrajFileCacheExists(lLineId, timestamp, strTrajMd5) == True) and (
                        os.path.exists(strStandardLocationFileTraj)):
                    rospy.logdebug("===enter switch (g_CacheUtil.CheckTrajFileCacheExists(lLineId, timestamp, strTrajMd5) == True) and (os.path.exists(strStandardLocationFileTraj))")
                    ...
                    rospy.logdebug("+++++++++++++++++++++++++++++intCacheModifyNameTraj:{0},intLocationTimeStamp:{1}".format(
                    ...
                    while True:
                        if intLocationTimeStamp == intCacheModifyNameTraj:
                            ...
                            if checkFileMd5(strStandardLocationFileTraj) == strTrajMd5:
                                rospy.logdebug ("##########intLocationTimeStamp == intCacheModifyNameTraj ,  checkFileMd5(strStandardLocationFileTraj) == " \
                                      "strTrajMd5 ")
                                ### direct use
                        ...
                        if intLocationTimeStamp < intCacheModifyNameTraj:
                            ...
                            break
                        break
                rospy.logdebug("----------------------------------before switch (g_CacheUtil.CheckStopFileCacheExists(lLineId, timestamp, strStopMd5) == True) and (os.path.exists(strStandardLocationFileStop))")
                if (g_CacheUtil.CheckStopFileCacheExists(lLineId, timestamp, strStopMd5) == True) and (
                        os.path.exists(strStandardLocationFileStop)):
                    rospy.logdebug("enter switch (g_CacheUtil.CheckStopFileCacheExists(lLineId, timestamp, strStopMd5) == True) and (os.path.exists(strStandardLocationFileStop))")
                    intLocationTimeStamp = int(os.path.getmtime(strStandardLocationFileTraj))
                    if globalDictTaskRunningStatus != lLineId:
                        break
                    ...
                    while True:
                        if intLocationTimeStamp == intCacheModifyNameStop:
                            ...
                            if checkFileMd5(strStandardLocationFileStop) == strStopMd5:
                                rospy.logdebug("##########intLocationTimeStamp == intCacheModifyNameStop , checkFileMd5(strStandardLocationFileStop) == strStopMd5")
                                ### direct use
                        ...
                        break
                    break
                rospy.logdebug("---------------------------------------before switch os.path.exists(strStandardLocationFileStop) and os.path.exists(strStandardLocationFileTraj)")
                ...
                if os.path.exists(strStandardLocationFileStop) and os.path.exists(strStandardLocationFileTraj):
                    rospy.logdebug("====enter switch  os.path.exists(strStandardLocationFileStop) and os.path.exists(strStandardLocationFileTraj)")
                    if globalDictTaskRunningStatus != lLineId:
                        break
                    intDownCompleteStopStatus = 3
                    intDownCompleteTrajStatus = 3
            ...
            while True:
                ...
                if (intDownCompleteTrajStatus == 2) or (intDownCompleteStopStatus == 2):
                    rospy.logdebug("##########intDownCompleteTrajStatus == False   or  intDownCompleteStopStatus == False happend")
                    #print "============================================================================================="
                    # SaveEventToFile(msg='', code='ISYS_INIT_TRAJECTORY_FAILURE', results=list(), actions=list(),level='info')
                    intProcessRet = 1
                    rospy.logdebug("=====ISYS_INIT_TRAJECTORY_FAILURE")
                    break
...
def call_process(msg):
    try:
        ...
        rospy.logdebug("type:{0}".format(type(pbLine.line.trajMd5)))
        strTempTrajMd5 = str(pbLine.line.trajMd5)
        strTrajMd5 = strTempTrajMd5
        rospy.logdebug("!!!!!!!!!!!!!!!!!!!!!strTrajMd5:{0}".format(strTrajMd5))
        ...
        rospy.logdebug("call_process: recv  lLineId:{0}, strTrajUrl:{1}, strTrajMd5:{2}, strStopUrl:{3},strStopMd5:{4},timestamp:{5},strVersion:{6}".format(
        ...
        rospy.logdebug("pbSend.sync_status:  {0}".format(intResultProcess))
        ...
        if globalDictTaskRunningStatus == (int(lLineId)):
            rospy.logdebug("success delete key from  globalDictTaskRunningStatus: {0}".format(int(lLineId)))
            globalDictTaskRunningStatus = -1
            rospy.logdebug("now  display globalDictTaskRunningStatus:{0}".format(globalDictTaskRunningStatus))
...
def kill_proc_tree(pid, sig=signal.SIGKILL, include_parent=True,
    ...
    if pid == os.getpid():
        rospy.logwarn("won't kill myself")
        return
    try:
...
def topicMsgCallback(msg):
    rospy.logdebug("--------------------------------------------------recv from channel /trajectory_agent/cmd/transaction ")
    global globalDictTaskRunningStatus
    if msg.size > 0:
        pbLine = common_message_pad_pb2.TrajectoryDownloadReq()
        pbLine.ParseFromString(msg.data)
        lLineId = pbLine.line.lineId
        if globalDictTaskRunningStatus == lLineId:
            rospy.logdebug("---------recv Task ---ignore---------globalDictTaskRunningStatus has same lineId:{0} ,ingore execute task".format(
                int(lLineId)))
            pass
        else:
            rospy.logdebug("---------recv Task ---recv---------globalDictTaskRunningStatus not  has same lineId:{0} ,execute task".format(
                int(lLineId)))
        ...
        rospy.logdebug("current globalDictTaskRunningStatus:{0}".format(globalDictTaskRunningStatus))
