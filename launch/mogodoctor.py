...
def check_telematics_log():
    ...
    cmd = "sed -n -e '/%s/h' -e '/%s/H;${x;p}' %s"%(ap_start_kw,ap_stop_kw,logFile)
    cmd_stdout = _exec_cmd(cmd)
    if not cmd_stdout or cmd_stdout == '\n':
        logerr("telematics hasn't received autopilot command")
        return False,'',''
    
    mostp = re.search(r"\"startLatLon\":\{.*?\}",cmd_stdout)
    moedp = re.search(r"\"endLatLon\":\{.*?\}",cmd_stdout)
    startll = json.loads('{%s}'%mostp.group(0))
    endll = json.loads('{%s}'%moedp.group(0))
    stlon = startll['startLatLon']['lon']
    stlat = startll['startLatLon']['lat']
    edlon = endll['endLatLon']['lon']
    edlat = endll['endLatLon']['lat']
    ...
    for line in cmd_stdout.split('\n'):
        remod = re.search(r'\d{10}\.\d+',line)
        if remod: 
            time_list.append(float(remod.group()))
    loginfo("traj:%s\nstop:%s"%(traj_filename,stop_filename))
    if cmd_stdout.count('\n') >= 2:
        endtime = ''
        if len(time_list) > 1:
            endtime = time_list[1]
        logerr("autopilot has exit by user request.start time:%s, end time:%s"%(_transform_timestamp(time_list[0]),_transform_timestamp(endtime)))
        return False,traj_filename,stop_filename
    else:
        # 获取起点终点坐标
        loginfo("receive autopilot at %s"%_transform_timestamp(time_list[0]))
